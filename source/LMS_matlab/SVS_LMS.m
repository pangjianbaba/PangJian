...
Noise=round(noise*2^(N-2)); % N bit 量化噪声信号
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
x=round(Xn*2^(N-2)); %N bit量化,如果有n个信号相加，则设置（N-n） %噪声信号+原始信号
d=y_s;   %以量化后的原始信号为参考信号

%%%%%%%%%%%%%量化后的 加噪信号写入文本
fid=fopen('E:\FPGA_practice\Vivado\LMS_filter\Xn_noise.txt','W');  %把数据写入文件中，
for i=1:length(x)
    if(x(i)>=0)
...
fclose(fid);
%%%%%%%%量化后的原始信号写入文本%%%%
fid=fopen('E:\FPGA_practice\Vivado\LMS_filter\Xn.txt','W');  %把数据写入文件中，
for a=1:length(Y_N)
    if(Y_N(a)>=0)
